...
> Make a table using the Table insert command with two rows and two columns in the Comparing sodium and calories section of the document. Caption the table as `Summary comparison between sodium and calories`
...
> Add the command to load the `knitr` library to your setup code block. Now filter your dataset to only show the highest sodium items and make a table using `kable()` in the High sodium items section. Remember the special labels that table code blocks must have. Render the document to verify this worked.
