...
>
> From the articles, we know that mileage, condition, accident history will affect the value of a used vehicle. Condition is more subjective than mileage --- someone selling a reliable, accident-free car with paint scratches and surface rust might describe it as excellent, whereas most buyers might call it good to average. Thus, I expect:
>
...
> **Points of emphasis:**
>
> -   Any reasonable articles work here.
...
kable(summary(vehicles.pop[, c("price", "odometer", "year")])) %>% kable_styling(full_width=F)
 
kable(table(vehicles.pop$origin),col.names = c("origin", "frequency"))%>% kable_styling(full_width=F)

kable(table(vehicles.pop$type),col.names = c("type", "frequency"))%>% kable_styling(full_width=F)
...
kable(colSums(is.na(vehicles.tidy[, c("price", "odometer", "year", "type", "origin")])),col.names=c("NA count")) %>% kable_styling(full_width=F)
...
> Here I assume the data are **MAR**.
>
> -   First, I determined the data are not MCAR because after visualizing missingness, we can see the probability of being missing is not the same for all cases.
>
> -   Second, this data are most possible to be MAR. Because I can tell from the graph that when `origin` is missing, `price` is less likely to be missing.
>
> -   Third, I ackowledge it is likely the data are NMAR, for example, to value a used car, the dealer needs abundant experience to make managerial judgements. Missing `price` could be due to junior evaluators' refusal to answer.
>
> MAR influence on our confidence interval and hypothesis test estimates: To make the MAR condition more plausible in the statistical inferential process, we should **include** the variables that could explain missingness, which in my analysis is `origin`.

```{r}
#| label: q2b
#| fig-cap: "Visulization of missingness"
#| fig-cap-location: top

vehicles.tidy %>% sample_frac(0.1) %>% vis_miss(cluster = TRUE)
...
> -   There are many ways to explore MCAR, MAR, NMAR, efforts on analyzing and justifying your arguments will have full points.
...
> -   **Randomization**: yes, I made an assumption here that our sample is representative.
> -   **10% condition**: yes, each sample is less than 10% of the population.
...
> The sampling frame in this case is all used cars in Springfield, Massachusetts listed on Craigslist during 2021.
...
```{r}
#|label: q3b1
ggplot(vehicles.springfield, aes(x=price)) + geom_histogram(fill="lightblue",color="darkblue")+labs(title="Price distribution of used cars in Springfield",x="Price in USD", y="Frequency")
```
...
>
> $$
> \bar{x}\pm t_{0.95, n-1} \frac{s}{\sqrt{n}}=24147.17\pm 1.65 \times \frac{22174.29}{\sqrt{224}}=(21700.02,26594.32)
> $$
...
> -   **Randomization condition**: yes, prices are mutually independent.
> -   **Nearly normal condition**: not really. However, due to n\>40, t -methods are safe because our histogram is not extremely skewed.

-   Interpret your confidence interval

> We are 95% confident that the true mean price is between 21700.02 and 26594.32.
...
-   What do you think is a reasonable critical value to select in this case? Choose your own critical value for your hypotheses tests. \> Answers may vary, remember that a critical value is the $z^*$ value, not $\alpha$. Here I assume the critical value is -1.96 or 1.96, and the coresponding confidence level is 0.95.
...
> -   As to **practical significance**, This small $p$ provides strong evidence against H0. I am convinced of the difference of proportion of regular cars sold in Springfield compared to the population.

-   What are some possible lurking variables that might make our conclusion unreliable?

> The proportion of regular cars sold may vary depending on economic factors such as income, employment, and gas prices. A higher-priced gas may discourage people from buying large cars, for example.
...
> It depends on the significant level. For example, we cannot reject the null hypothesis at a significance level of 0.05, but we can reject the null hypothesis at the 0.1 significance level.
...
> -   **Randomization and Independence**: yes
> -   **10% condition**: yes
...
>
> The p-value is **0.9999976 \> 0.1, failed to reject H0**. That means there is no sufficient evidence for us to say average odometer is higher than the population at an alpha of 0.10.
...
> -   **A type I**: null is true but we mistakenly reject it. We conclude that the age is different from 12.2 when it is 12.2.
> -   **A type II**: the alternative is true but we mistakenly fail to reject the null hypothesis. We conclude the age is 12.2 when actually the age is different from 12.2.
...
> -   **For example, a type II error would be more serious** than a Type I error. Because this could mean that the analyst fails to identify important trends or patterns in the market, leading to business decisions to remain the old strategy and not innovate, which may make them fall behind their competitors.
...
> -   Answers may vary.
> -   I think it depends on the objective of the data. For me, the size of difference should be 2 standard deviation away from the mean, just by convention.
...
> The p-value is 0.008 \< 0.05, we can **reject the null hypothesis**.
...
p.boston <- (1-pnorm(z))*2

```

-   Explain in this context what your $p$ value means.

> we have calculated a p value of 0.008. This means that **assuming the null hypothesis were true**, we would expect to see 0.8% of samples have a difference this large or larger between the two groups just by chance.
...
ggplot(vehicles.springfield, aes(x=price)) + geom_histogram(fill="lightblue",color="darkblue")+labs(title="Distribution of price at Springfield", x="Price in USD", y="Frequency")
ggplot(boston, aes(x=price)) + geom_histogram(fill="lightblue",color="darkblue")+labs(title="Distribution of price at Boston", x="Price in USD", y="Frequency")
```
